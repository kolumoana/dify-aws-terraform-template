...
resource "aws_ssm_parameter" "dify_init_password" {
  type  = "SecureString"
  name  = "${local.ssm_parameter_prefix}/INIT_PASSWORD"
  value = var.dify_init_password
  lifecycle {
    ignore_changes = [value]
  }
}
  ...
  container_definitions = jsonencode([
    {
      ...
      secrets = [
        {
          name      = "INIT_PASSWORD"
          valueFrom = aws_ssm_parameter.dify_init_password.name
        },
        {
          name      = "SECRET_KEY"
...
resource "aws_security_group" "vpce" {
  ...
  ingress {
    ...
    to_port         = 443
    protocol        = "tcp"
    security_groups = [aws_security_group.api.id, aws_security_group.web.id, aws_security_group.worker.id]
  }
...
resource "aws_nat_gateway" "this" {
  ...
  subnet_id     = var.public_subnet_ids[0] # パブリックサブネットの最初のものを使用
...
# プライベートサブネット用のルートテーブルにNATゲートウェイ経由のルートを追加
